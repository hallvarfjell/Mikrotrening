...
(function(){
  ...
  function collect(){
    const series=[]; listEl.querySelectorAll('.menu-item').forEach(row=>{
      series.push({
        reps:Number(row.querySelector('.sr-reps').value||0),
        workSec:Number(row.querySelector('.sr-work').value||0),
        restSec:Number(row.querySelector('.sr-rest').value||0),
        seriesRestSec:Number(row.querySelector('.sr-srest').value||0)
      });
    });
    return {
      name: nameEl.value||'Custom',
      // store in SECONDS (minutes * 60)
      warmupSec: Number(warmEl.value||0)*60,
      cooldownSec: Number(coolEl.value||0)*60,
      targetSpeed: tSpd.value?Number(tSpd.value):null,
      targetGrade: tGrd.value?Number(tGrd.value):null,
      series
    };
  }
  function totalSeconds(cfg){
    const warm=Number(cfg.warmupSec||0);
    const cool=Number(cfg.cooldownSec||0);
    const series=cfg.series||[];
    const sum=series.reduce((a,s)=> a + (Number(s.reps||0)*(Number(s.workSec||0)+Number(s.restSec||0))) + Number(s.seriesRestSec||0), 0);
    return warm + sum + cool;
  }
  function fmtMMSS(sec){ sec=Math.max(0,Math.floor(sec)); const m=Math.floor(sec/60), s=String(sec%60).padStart(2,'0'); return `${m}:${s}`; }
  function preview(){ const cfg=collect(); const tot=totalSeconds(cfg); previewEl.textContent=`Varighet: ${fmtMMSS(tot)} — ${cfg.series.length} serier — Oppv ${fmtMMSS(cfg.warmupSec)}, Nedjogg ${fmtMMSS(cfg.cooldownSec)}`; }

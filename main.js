...
// W ≈ m*v*(g*grade + C), where C ≈ 4.186 J/(kg·m) ~ 1 kcal·kg⁻¹·km⁻¹, v in m/s, grade in fraction.
function estimateWatt(speedKmh, gradePct, massKg){
  const g=9.81; const v=speedKmh/3.6; const grade=(gradePct||0)/100; const C=4.186; // baseline metabolic cost per meter
  const w = massKg * v * (g*grade + C);
...
function tick(){ const t=Date.now(); if(STATE.hr!=null) STATE.series.hr.push({t,y:STATE.hr}); STATE.series.speed.push({t,y:STATE.speedKmh}); const w=estimateWatt(STATE.speedKmh, STATE.gradePct, STATE.massKg); STATE.series.watt.push({t,y:w}); const cutoff=t-STATE.windowSec*1000; for(const k of ['hr','speed','watt']){ const arr=STATE.series[k]; while(arr.length && arr[0].t<cutoff) arr.shift(); } el('pulse').textContent = STATE.hr!=null?STATE.hr:'--'; el('watt').textContent = w||'--'; const s=calcSlope(); el('slope').textContent = s!=null ? (s>0?`+${s}`:`${s}`) : '--'; draw(); }

...
// ---- Debug helper ----
function dbg(){ const box=document.getElementById('dbg'); if(!box) return; const s=STATE.workout; const sessions=JSON.parse(localStorage.getItem('sessions')||'[]'); box.textContent=JSON.stringify({now:new Date().toISOString(), loggerActive:STATE.logger.active, points:STATE.logger.points.length, workoutPhase:s? s.phase:null, sessions:sessions.length}, null, 2);} setInterval(dbg,1000);
const genBtn=document.getElementById('gen-test'); if(genBtn){ genBtn.onclick=()=>{ const t0=Date.now()-600000; const pts=[]; let dist=0; for(let i=0;i<600;i++){ const ts=t0+i*1000; const sp=(i%60<30?14:8); const ms=sp/3.6; dist+=ms; pts.push({ts,iso:new Date(ts).toISOString(),hr:140+Math.round(20*Math.sin(i/15)),speed_ms:ms,grade:(i%120<60?2:0),dist_m:dist,rpe:(i%300<240?7:3),phase:(i<60?'warmup': i<540?(i%60<30?'work':'rest'):'cooldown'),rep:Math.floor((i-60)/60)+1,watt:250+Math.round(50*Math.sin(i/10))}); } const sess={id:'s'+Date.now(),name:'Testøkt',reps:8,startedAt:new Date(t0).toISOString(),endedAt:new Date(t0+599000).toISOString(),lt1:STATE.LT1,lt2:STATE.LT2,massKg:STATE.massKg,rpeByRep:{},points:pts}; const arr=JSON.parse(localStorage.getItem('sessions')||'[]'); arr.push(sess); localStorage.setItem('sessions', JSON.stringify(arr)); alert('La inn testøkt. Åpne resultatsida.'); }}

// ---- Wake Lock ----
async function requestWakeLock(){ try{ if('wakeLock' in navigator){ STATE.wakeLock=await navigator.wakeLock.request('screen'); STATE.wakeLock.addEventListener('release',()=>{STATE.wakeLock=null;}); } }catch(e){ console.warn('WakeLock failed',e);} } document.addEventListener('visibilitychange',()=>{ if(document.visibilityState==='visible'&&!STATE.wakeLock){ requestWakeLock(); }}); requestWakeLock();
...
// ---- Series + UI periodic updates + Logger 1 Hz ----
function tick(){ const t=Date.now(); if(STATE.hr!=null) STATE.series.hr.push({t,y:STATE.hr}); const dispSpeed=displaySpeedKmh(); STATE.series.speed.push({t,y:dispSpeed}); const w=estimateWatt(dispSpeed, STATE.gradePct, STATE.massKg); STATE.series.watt.push({t,y:w}); const cutoff=t-STATE.windowSec*1000; for(const k of ['hr','speed','watt']){ const arr=STATE.series[k]; while(arr.length && arr[0].t<cutoff) arr.shift(); } if(el('pulse')) el('pulse').textContent=STATE.hr!=null?STATE.hr:'--'; if(el('watt')) el('watt').textContent=w||'--'; const s=calcSlope(); if(el('slope')) el('slope').textContent=(s!=null)?(s>0?`+${s}`:`${s}`):'--'; draw(); if(STATE.logger.active){ const prevTs=STATE.logger.points.length? STATE.logger.points[STATE.logger.points.length-1].ts : STATE.logger.startTs; const dt=(t-prevTs)/1000; const speed_ms=dispSpeed/3.6; STATE.logger.dist += speed_ms*dt; const wstate=STATE.workout; STATE.logger.points.push({ ts:t, iso:new Date(t).toISOString(), hr:STATE.hr||0, speed_ms, grade:STATE.gradePct||0, dist_m:STATE.logger.dist, rpe:STATE.rpe, phase:wstate?wstate.phase:'', rep:wstate&&wstate.phase==='work'?wstate.rep:0, watt:w }); } }
...
function draw(){ if(!ctx||!canvas) return; const W=canvas.width,H=canvas.height; ctx.clearRect(0,0,W,H); const padL=60*dpr,padR=20*dpr,padT=20*dpr,padB=30*dpr; const plotW=W-padL-padR, plotH=H-padT-padB; const now=Date.now(); const xmin=now-STATE.windowSec*1000, xmax=now; const showHR=el('show-hr')?el('show-hr').checked:true, showWatt=el('show-watt')?el('show-watt').checked:true, showSpeed=el('show-speed')?el('show-speed').checked:true; const hrMin=Number(el('hr-min')?el('hr-min').value:80)||80, hrMax=Number(el('hr-max')?el('hr-max').value:200)||200; const yHR=v=> padT + (1 - (v-hrMin)/(hrMax-hrMin))*plotH; function band(y0,y1,color){ ctx.fillStyle=color; ctx.fillRect(padL,y1,plotW,y0-y1);} band(yHR(hrMin), yHR(STATE.LT1), '#edf7ed'); band(yHR(STATE.LT1), yHR(STATE.LT2), '#e9f0fb'); band(yHR(STATE.LT2), yHR(hrMax), '#fdeaea'); ctx.strokeStyle='#e5e7eb'; ctx.lineWidth=1; ctx.beginPath(); for(let sec=0; sec<=STATE.windowSec; sec+=60){ const t=xmin+sec*1000; const x=padL+(t-xmin)/(xmax-xmin)*plotW; ctx.moveTo(x,padT); ctx.lineTo(x,padT+plotH);} ctx.stroke(); ctx.strokeStyle='#e2e8f0'; ctx.beginPath(); for(let v=hrMin; v<=hrMax; v+=10){ const y=yHR(v); ctx.moveTo(padL,y); ctx.lineTo(padL+plotW,y);} ctx.stroke(); function xTime(t){ return padL + (t-xmin)/(xmax-xmin)*plotW; } function drawLine(arr,color,ymap,alpha=1){ if(!arr || arr.length<2) return; ctx.strokeStyle=color; ctx.globalAlpha=alpha; ctx.lineWidth=2*dpr; ctx.beginPath(); let moved=false; for(const p of arr){ if(p.t<xmin) continue; const x=xTime(p.t), y=ymap(p.y); if(!moved){ ctx.moveTo(x,y); moved=true;} else ctx.lineTo(x,y);} ctx.stroke(); ctx.globalAlpha=1; } if(showHR) drawLine(STATE.series.hr, '#16a34a', yHR, 1); const sp=STATE.series.speed, wt=STATE.series.watt; const spVals=sp.map(p=>p.y), wtVals=wt.map(p=>p.y); let gWtVals=[]; if(STATE.ghost.enabled && STATE.ghost.avg && STATE.ghost.avg.watt){ for(const gp of STATE.ghost.avg.watt){ if(gp.t<xmin || gp.t>now) continue; gWtVals.push(gp.y); } } const wmin=Math.min(...(wtVals.length?wtVals:[0]), ...(gWtVals.length?gWtVals:[0])); const wmax=Math.max(...(wtVals.length?wtVals:[1]), ...(gWtVals.length?gWtVals:[1])); const smin=Math.min(...(spVals.length?spVals:[0])); const smax=Math.max(...(spVals.length?spVals:[1])); const mapSpeed=v=> yHR(hrMin + (hrMax-hrMin) * ((v - smin) / Math.max(1e-6,(smax-smin)))); const mapWatt=v => yHR(hrMin + (hrMax-hrMin) * ((v - wmin) / Math.max(1e-6,(wmax-wmin)))); if(showSpeed) drawLine(sp, '#2563eb', mapSpeed, 1); if(showWatt) drawLine(wt, '#d97706', mapWatt, 1); if(STATE.ghost.enabled && STATE.ghost.avg){ if(showHR) drawLine(STATE.ghost.avg.hr, '#22c55e', yHR, 0.55); if(showWatt) drawLine(STATE.ghost.avg.watt, '#f59e0b', mapWatt, 0.55); }
}

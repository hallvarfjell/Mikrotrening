...
(function(){
  ...
  function init(){ try{
    ...
    el('btn-start-pause')?.addEventListener('click', async ()=>{
      ...
      if(!STATE.ticker){ if(STATE.hr==null){ const want=confirm('Koble til pulsbelte først?'); if(want){ await connectHR(); return; } } startTicker(); } else { stopTicker(); }
    });

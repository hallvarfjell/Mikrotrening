...
(function(){
  ...
  const RPE_TXT = {"1": "Ingen anstrengelse", "2": "Svært lett", "3": "Lett", "4": "Moderat", "5": "Middels / kontrollert hard", "6": "Moderat hard", "7": "Hard", "8": "Svært hard", "9": "Nesten maksimal", "10": "Maksimal innsats"};
  ...
  function rpeDescription(v){ const n=Math.round(clamp(Number(v)||0,0,10)); return (n in RPE_TXT)? `RPE ${n} – ${RPE_TXT[n]}` : 'RPE 0–10 – velg et nivå'; }
  function setRPE(v){ STATE.rpe = clamp(Number(v)||0,0,10); if(el('rpe-now')) el('rpe-now').value = STATE.rpe.toFixed(1); const lab=el('rpe-desc'); if(lab) lab.textContent = rpeDescription(STATE.rpe); }
  ...
  function computeTotalDur(cfg){ const s=cfg.series||[]; let total=Number(cfg.warmupSec||0)+Number(cfg.cooldownSec||0); for(const x of s){ total += Number(x.reps||0) * (Number(x.workSec||0) + Number(x.restSec||0)); total += Number(x.seriesRestSec||0); } return total; }

  function populateWorkoutSelect(){ const sel=el('workout-select'); if(!sel) return; sel.innerHTML=''; const customs=loadCustomWorkouts(); customs.forEach((cw,idx)=>{ const opt=document.createElement('option'); opt.value='c:'+idx; opt.textContent=cw.name||('Mal '+(idx+1)); sel.appendChild(opt); }); sel.addEventListener('change', ()=>{ const v=sel.value; if(v && v.startsWith('c:')){ const idx=Number(v.split(':')[1]); const cw=loadCustomWorkouts()[idx]; if(cw){ STATE.workout=workoutFromCfg(cw); updateWorkoutUI(); setNS('lastPreset','custom:'+idx); const sd=el('sel-dur'); if(sd) sd.textContent = fmtMMSS(computeTotalDur(cw)); } } }); }

  function preselectIfRequested(){ const ar=getNS('preselect',null); if(!ar) return; if(ar.type==='custom'){ const arr=loadCustomWorkouts(); const i=Number(ar.index||0); const cw=arr[i]; if(cw){ const sel=el('workout-select'); if(sel){ sel.value='c:'+i; } STATE.workout=workoutFromCfg(cw); updateWorkoutUI(); const sd=el('sel-dur'); if(sd) sd.textContent = fmtMMSS(computeTotalDur(cw)); } } delNS('preselect'); }
  ...
  function init(){ try{
    for(const a of (document.getElementById('topbar')?.querySelectorAll('a')||[])) a.addEventListener('click', (e)=>{ if(STATE.workout && STATE.ticker && STATE.workout.phase!=='done'){ e.preventDefault(); alert('Avslutt økta før du navigerer bort fra hovedsida.'); } });
    window.addEventListener('beforeunload', (e)=>{ if(STATE.workout && STATE.ticker && STATE.workout.phase!=='done'){ e.preventDefault(); e.returnValue='Økta pågår. Avslutt før du lukker/navigerer bort.'; } });
    ...
    function openNoHRModal(){ const m=el('nohr-modal'); if(m) m.classList.add('open'); }
    function closeNoHRModal(){ const m=el('nohr-modal'); if(m) m.classList.remove('open'); }
    el('nohr-cancel')?.addEventListener('click', closeNoHRModal);
    el('nohr-start')?.addEventListener('click', ()=>{ closeNoHRModal(); startTicker(); });
    el('nohr-connect')?.addEventListener('click', async ()=>{ closeNoHRModal(); await connectHR(); });

    el('btn-start-pause')?.addEventListener('click', async ()=>{
      if(!STATE.workout){ const sel=el('workout-select'); const customs=loadCustomWorkouts(); if(sel && sel.value && sel.value.startsWith('c:')){ const idx=Number(sel.value.split(':')[1]); const cw=customs[idx]; if(cw){ STATE.workout=workoutFromCfg(cw); } else { return alert('Velg en økt.'); } } else { return alert('Velg en økt.'); } }
      if(!STATE.ticker){ if(STATE.hr==null){ openNoHRModal(); return; } startTicker(); } else { stopTicker(); }
    });
    ...
    el('btn-discard')?.addEventListener('click', ()=>{ if(confirm('Forkast økta (ikke lagre)?')){ if(STATE.ticker) clearInterval(STATE.ticker), STATE.ticker=null; STATE.workout=null; STATE.logger.active=false; STATE.logger.points=[]; updateWorkoutUI(); draw(); }});

    canvas=el('chart'); ctx=canvas?.getContext('2d'); dpr=window.devicePixelRatio||1; window.addEventListener('resize', resizeCanvas); resizeCanvas();

    el('ghost-picker')?.addEventListener('click', (e)=>{ e.stopPropagation(); el('ghost-menu')?.classList.remove('hidden'); buildGhostList(); });
    document.addEventListener('click', (e)=>{ const menu=el('ghost-menu'); const picker=el('ghost-picker'); if(menu && !menu.classList.contains('hidden') && !menu.contains(e.target) && e.target!==picker) menu.classList.add('hidden'); });
    ...
    el('ghost-apply')?.addEventListener('click', ()=>{ const checks=el('ghost-list')?.querySelectorAll('input[type=checkbox]'); STATE.ghost.ids=new Set(Array.from(checks||[]).filter(c=>c.checked).map(c=>c.value)); computeGhostAverage(); el('ghost-menu')?.classList.add('hidden'); });

    populateWorkoutSelect();
    preselectIfRequested();

    setInterval(()=>{ try{ const t=Date.now(); if(STATE.hr!=null) STATE.series.hr.push({t,y:STATE.hr}); const dispSpeed=displaySpeedKmh(); STATE.series.speed.push({t,y:dispSpeed}); const w=estimateWattExternal(dispSpeed, STATE.gradePct, STATE.massKg, STATE.cal.Crun, STATE.cal.K); STATE.series.watt.push({t,y:w}); STATE.series.rpe.push({t,y:STATE.rpe}); const cutoff=t-STATE.windowSec*1000; for(const k of ['hr','speed','watt','rpe']){ const arr=STATE.series[k]; while(arr.length && arr[0].t<cutoff) arr.shift(); } if(el('pulse')) el('pulse').textContent = (STATE.hr!=null?STATE.hr:'--'); if(el('watt')) el('watt').textContent = w||'--'; const s=(function(){ const s=STATE.series.hr; if(!s.length) return null; const now=Date.now(), lo20=now-20000, lo120=now-120000; let sum20=0,cnt20=0,sum120=0,cnt120=0; for(let i=s.length-1;i>=0;i--){ const p=s[i]; if(p.t>=lo20){ sum20+=p.y; cnt20++; } if(p.t>=lo120){ sum120+=p.y; cnt120++; } else break; } if(!cnt20||!cnt120) return null; return Math.round(sum20/cnt20 - sum120/cnt120); })(); if(el('slope')) el('slope').textContent=(s!=null)?(s>0?`+${s}`:`${s}`):'--'; draw(); if(STATE.logger.active){ writeSample(t); } } catch(e){ showErr(e); } },1000);

...
// ---- Prevent leaving during active workout ----
function isWorkoutRunning(){ return !!(STATE.workout && STATE.ticker && STATE.workout.phase!=='done'); }
function guardNav(e){ if(isWorkoutRunning()){ e.preventDefault(); alert('Avslutt økta før du navigerer bort fra hovedsida.'); return false; } }
...
function autorunIfRequested(){ const ar=getNS('autorun',null); if(!ar) return; if(ar.type==='custom'){ const arr=loadCustomWorkouts(); const i=Number(ar.index||0); const cw=arr[i]; if(cw){ // set select
    const sel=el('workout-select'); if(sel){ sel.value='c:'+i; }
    STATE.workout=workoutFromCfg(cw); updateWorkoutUI(); startTicker(); toggleStartPauseUI(true);
  }
}
delNS('autorun');

function init(){ populateWorkoutSelect(); autorunIfRequested(); }
...
// ---- Ghost drawing (unchanged from 9.12) ----
...
setInterval(()=>{ const t=Date.now(); if(STATE.hr!=null) STATE.series.hr.push({t,y:STATE.hr}); const dispSpeed=displaySpeedKmh(); STATE.series.speed.push({t,y:dispSpeed}); const w=estimateWattExternal(dispSpeed, STATE.gradePct, STATE.massKg, STATE.cal.Crun, STATE.cal.K); STATE.series.watt.push({t,y:w}); STATE.series.rpe.push({t,y:STATE.rpe}); const cutoff=t-STATE.windowSec*1000; for(const k of ['hr','speed','watt','rpe']){ const arr=STATE.series[k]; while(arr.length && arr[0].t<cutoff) arr.shift(); } if(STATE.logger.active){ writeSample(t); } if(el('pulse')) el('pulse').textContent = (STATE.hr!=null?STATE.hr:'--'); if(el('watt')) el('watt').textContent = w||'--'; const s=(()=>{ const s=STATE.series.hr; if(!s.length) return null; const avg=(series,ms)=>{ const lo=t-ms; let sum=0,cnt=0; for(let i=series.length-1;i>=0;i--){ const p=series[i]; if(p.t<lo) break; sum+=p.y; cnt++; } return cnt? sum/cnt : null; }; const a20=avg(s,20000), a120=avg(s,120000); if(a20==null||a120==null) return null; return Math.round(a20-a120); })(); if(el('slope')) el('slope').textContent=(s!=null)?(s>0?`+${s}`:`${s}`):'--'; draw(); },1000);

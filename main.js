...
// ---- Wake Lock ----
async function requestWakeLock(){ try{ if('wakeLock' in navigator){ STATE.wakeLock = await navigator.wakeLock.request('screen'); STATE.wakeLock.addEventListener('release', ()=>{ STATE.wakeLock=null; }); } }catch(e){ /* ignore */ }}
...
function saveCustomWorkouts(arr){ setNS(BUILDER_KEY, arr); }
function workoutFromCfg(cw){ return { name:cw.name||'Custom', phase:'warmup', startedAt:null, endedAt:null, warmupSec:Number(cw.warmupSec)||0, cooldownSec:Number(cw.cooldownSec)||0, series:(cw.series||[]).map(s=>({reps:s.reps,workSec:s.workSec,restSec:s.restSec,seriesRestSec:s.seriesRestSec,note:s.note||''})), sIdx:-1, rep:0, tLeft:Number(cw.warmupSec)||0 } }
...
function nextPhase(){ const w=STATE.workout; if(!w) return; if(w.phase==='warmup'){ if(w.series && w.series.length){ w.phase='work'; w.sIdx=0; w.rep=1; w.tLeft=w.series[0].workSec||0; if(w.tLeft===0){ // hoppe rett til rest hvis work=0 (pause-steg)
      w.phase='rest'; w.tLeft=w.series[0].restSec||0; }
    } else { w.phase='cooldown'; w.tLeft=w.cooldownSec; } return; }
 if(w.phase==='work'){ const s=w.series[w.sIdx]; if(w.rep < s.reps){ // pause etter drag
    w.phase='rest'; w.tLeft=s.restSec||0; return; }
  if(w.sIdx < w.series.length-1){ const sr=s.seriesRestSec||0; if(sr>0){ w.phase='seriesrest'; w.tLeft=sr; return; } w.sIdx++; w.phase='work'; w.rep=1; w.tLeft=w.series[w.sIdx].workSec||0; if(w.tLeft===0){ w.phase='rest'; w.tLeft=w.series[w.sIdx].restSec||0; } return; }
  w.phase='cooldown'; w.tLeft=w.cooldownSec; return; }
 if(w.phase==='rest'){ const s=w.series[w.sIdx]; if(w.rep < s.reps){ w.rep++; w.phase='work'; w.tLeft=s.workSec||0; if(w.tLeft===0){ // hvis pause-steg med work=0 -> gå rett til rest (men da er rep brukt opp)
      if(w.rep<=s.reps){ w.phase='rest'; w.tLeft=s.restSec||0; } }
    return; }
  if(w.sIdx < w.series.length-1){ const sr=s.seriesRestSec||0; if(sr>0){ w.phase='seriesrest'; w.tLeft=sr; return; } w.sIdx++; w.phase='work'; w.rep=1; w.tLeft=w.series[w.sIdx].workSec||0; if(w.tLeft===0){ w.phase='rest'; w.tLeft=w.series[w.sIdx].restSec||0; } return; }
  w.phase='cooldown'; w.tLeft=w.cooldownSec; return; }
...
function prevPhase(){ const w=STATE.workout; if(!w) return; // simple back logic as before
 if(w.phase==='work'){ const s=w.series[w.sIdx]; if(w.rep>1){ w.phase='rest'; w.rep--; w.tLeft=s.restSec||0; }
 else { if(w.sIdx>0){ w.sIdx--; const ps=w.series[w.sIdx]; w.phase='work'; w.rep=ps.reps; w.tLeft=ps.workSec||0; if(w.tLeft===0){ w.phase='rest'; w.tLeft=ps.restSec||0; } } else { w.phase='warmup'; w.tLeft=w.warmupSec; } }
 }
...
document.addEventListener('click', (e)=>{ const menu=el('ghost-menu'); if(!menu) return; const picker=el('ghost-picker'); if(!menu.classList.contains('hidden') && !menu.contains(e.target) && e.target!==picker) closeGhostMenu(); });
...
function computeGhostAverage(){ const ids=Array.from(STATE.ghost.ids||[]); const sessions=getNS('sessions',[]).filter(s=> ids.includes(s.id)); if(!sessions.length){ STATE.ghost.avg=null; return; }
...
function draw(){ if(!ctx||!canvas) return; const W=canvas.width,H=canvas.height; const padL=60*dpr,padR=60*dpr,padT=30*dpr,padB=24*dpr; const plotW=W-padL-padR, plotH=H-padT-padB; ctx.clearRect(0,0,W,H); if(plotW<=0||plotH<=0) return; const now=Date.now(); const xmin=now-STATE.windowSec*1000, xmax=now; const showHR=el('show-hr')?.checked ?? getNS('defHR',true); const showWatt=el('show-watt')?.checked ?? getNS('defWatt',true); const showSpeed=el('show-speed')?.checked ?? getNS('defSpeed',false); const showRPE=el('show-rpe')?.checked ?? getNS('defRPE',true);
  const hrMin= getNS('hrMin',80), hrMax=getNS('hrMax',200); const yHR=v=> padT + (1 - (v-hrMin)/(hrMax-hrMin||1))*plotH;
...
function finishSession(){ try{ const w=STATE.workout; if(!w) return; const nowIso=new Date().toISOString(); if(!w.startedAt) w.startedAt = STATE.logger.startTs? new Date(STATE.logger.startTs).toISOString(): nowIso; if(!w.endedAt) w.endedAt=nowIso; if(STATE.logger.points.length<2){ const t0=STATE.logger.startTs || Date.now(); const t1=Date.now(); if(STATE.logger.points.length===0) writeSample(t0); writeSample(t1); }
  ...
  const arr=getNS('sessions',[]); arr.push(session); setNS('sessions', arr); window.location.assign('results.html#'+session.id);
} catch(e){ console.error('finishSession failed', e); alert('Klarte ikke å lagre økt: '+e.message); } }

['show-hr','show-watt','show-speed','show-rpe'].forEach(id=>{ const elc=el(id); if(!elc) return; const defKey={ 'show-hr':'defHR','show-watt':'defWatt','show-speed':'defSpeed','show-rpe':'defRPE' }[id]; elc.checked = getNS(defKey, id!=='show-speed'); elc.addEventListener('change', ()=> draw()); });

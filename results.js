...
document.addEventListener('DOMContentLoaded', function(){
  try{
    ...
    const chart=document.getElementById('r-chart'); resizeCanvas(chart); const toggles={hr:true,watt:true,speed:true}; document.getElementById('r-show-hr').addEventListener('change', e=>{ toggles.hr=e.target.checked; drawGraph(); }); document.getElementById('r-show-watt').addEventListener('change', e=>{ toggles.watt=e.target.checked; drawGraph(); }); document.getElementById('r-show-speed').addEventListener('change', e=>{ toggles.speed=e.target.checked; drawGraph(); });

    function drawGraph(){ const ctx=chart.getContext('2d'); const W=chart.width,H=chart.height; ctx.clearRect(0,0,W,H); const padL=60*dpr,padR=20*dpr,padT=20*dpr,padB=30*dpr; const plotW=W-padL-padR, plotH=H-padT-padB; if(!pts.length) return; const tmin=pts[0].ts, tmax=pts[pts.length-1].ts; const hrVals=pts.map(p=>p.hr).filter(Boolean); const hrMin=hrVals.length?Math.min(...hrVals):80; const hrMax=hrVals.length?Math.max(...hrVals):200; const yHR=v=> padT+(1-(v-hrMin)/(hrMax-hrMin||1))*plotH; function band(y0,y1,color){ ctx.fillStyle=color; ctx.fillRect(padL,y1,plotW,y0-y1);} band(yHR(hrMin),yHR(s.lt1),'#edf7ed'); band(yHR(s.lt1),yHR(s.lt2),'#e9f0fb'); band(yHR(s.lt2),yHR(hrMax),'#fdeaea'); ctx.strokeStyle='#e5e7eb'; ctx.lineWidth=1; ctx.beginPath(); for(let sec=0;sec<=Math.max(60,(tmax-tmin)/1000);sec+=60){ const tt=tmin+sec*1000; const x=padL+(tt-tmin)/(tmax-tmin||1)*plotW; ctx.moveTo(x,padT); ctx.lineTo(x,padT+plotH);} ctx.stroke(); ctx.strokeStyle='#e2e8f0'; ctx.beginPath(); for(let v=hrMin; v<=hrMax; v+=10){ const y=yHR(v); ctx.moveTo(padL,y); ctx.lineTo(padL+plotW,y);} ctx.stroke(); const xTime=t=> padL+(t-tmin)/(tmax-tmin||1)*plotW; function drawLine(arr,get,color,ymap){ if(arr.length<2) return; ctx.strokeStyle=color; ctx.lineWidth=2*dpr; ctx.beginPath(); let moved=false; for(const p of arr){ const val=get(p); if(val==null) continue; const x=xTime(p.ts), y=ymap(val); if(!moved){ ctx.moveTo(x,y); moved=true;} else ctx.lineTo(x,y);} ctx.stroke(); } if(toggles.hr) drawLine(pts, p=>p.hr, '#16a34a', yHR); const spVals=pts.map(p=>p.speed_ms*3.6), wtVals=pts.map(p=>p.watt); const smin=Math.min(...(spVals.length?spVals:[0])), smax=Math.max(...(spVals.length?spVals:[1])); const wmin=Math.min(...(wtVals.length?wtVals:[0])), wmax=Math.max(...(wtVals.length?wtVals:[1])); const mapSpeed=v=> yHR(hrMin + (hrMax-hrMin) * ((v - smin) / Math.max(1e-6,(smax-smin)))); const mapWatt=v => yHR(hrMin + (hrMax-hrMin) * ((v - wmin) / Math.max(1e-6,(wmax-wmin)))); if(toggles.speed) drawLine(pts, p=>p.speed_ms*3.6, '#2563eb', mapSpeed); if(toggles.watt) drawLine(pts, p=>p.watt, '#d97706', mapWatt); }
    drawGraph();

    const table=document.getElementById('laps'); const rows=[]; for(let r=1;r<= (s.reps||0); r++){ const arr=pts.filter(p=>p.phase==='work' && p.rep===r); if(!arr.length) continue; const m={rep:r, hr:Math.round(avg(arr.map(p=>p.hr))||0), spd:avg(arr.map(p=>p.speed_ms))*3.6, grd:avg(arr.map(p=>p.grade)), wat:Math.round(avg(arr.map(p=>p.watt))||0), rpe:(s.rpeByRep && s.rpeByRep[r]!=null)? s.rpeByRep[r]:'' }; rows.push(m);} table.innerHTML = `<thead><tr><th>Rep</th><th>Snittpuls</th><th>Snittfart</th><th>Snittwatt</th><th>Stigning</th><th>RPE</th></tr></thead>` + `<tbody>`+ rows.map(r=>`<tr><td>${r.rep}</td><td>${r.hr} bpm</td><td>${r.spd?r.spd.toFixed(1):'--'} km/t</td><td>${r.wat} W</td><td>${r.grd?r.grd.toFixed(1):'--'} %</td><td>${r.rpe}</td></tr>`).join('') +`</tbody>`;

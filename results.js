...
document.addEventListener('DOMContentLoaded', function(){
  try{
    ...
    if(!s){ document.getElementById('no-session').classList.remove('hidden'); document.getElementById('result-summary-card').classList.add('hidden'); return; }
    ...
    function drawGraph(){
      ...
      const padL=60*dpr,padR=60*dpr,padT=30*dpr,padB=30*dpr; const plotW=W-padL-padR, plotH=H-padT-padB;
      ...
      const rpeVals=rp.map(p=>p.y).filter(v=>!isNaN(v));
      ...
      ctx.fillStyle='rgba(96,165,250,0.06)'; ctx.fillRect(padL, yHR(s.lt1), plotW, yHR(s.lt2)-yHR(s.lt1));
      ctx.fillStyle='rgba(34,197,94,0.06)'; ctx.fillRect(padL, yHR(hrMin), plotW, yHR(s.lt1)-yHR(hrMin));
      // time grid
      ctx.strokeStyle='#374151'; ctx.lineWidth=1; ctx.beginPath(); for(let sec=0; sec<=Math.max(60,(tmax-tmin)/1000); sec+=60){ const tt=tmin+sec*1000; const x=padL+(tt-tmin)/(tmax-tmin||1)*plotW; ctx.moveTo(x,padT); ctx.lineTo(x,padT+plotH);} ctx.stroke();
      // HR ticks
      ctx.strokeStyle='#1f2937'; ctx.beginPath(); for(let v=hrMin; v<=hrMaxAxis; v+=10){ const y=yHR(v); ctx.moveTo(padL,y); ctx.lineTo(padL+plotW,y);} ctx.stroke();
      ...
      // Right axis Watt
      if(toggles.watt){ ctx.fillStyle='#22c55e'; ctx.textAlign='right'; const ticks=5; for(let i=0;i<=ticks;i++){ const v=wmin + (wmax-wmin)*i/ticks; const y=yWatt(v); ctx.fillText(String(Math.round(v)), W-8*dpr, y+4*dpr); } ctx.textAlign='left'; }
      // Top axis Speed
      if(toggles.speed){ ctx.fillStyle='#60a5fa'; ctx.textAlign='center'; const ticks=5; for(let i=0;i<=ticks;i++){ const v=smin + (smax-smin)*i/ticks; const x=padL + plotW*i/ticks; ctx.fillText(String(v.toFixed(1)), x, (padT-8*dpr)); } ctx.textAlign='left'; }
      // Right-inner axis RPE
      if(toggles.rpe){ ctx.fillStyle='#facc15'; ctx.textAlign='right'; for(let v=0; v<=10; v+=2){ const y=yRPE(v); ctx.fillText(String(v), W-40*dpr, y+4*dpr); } ctx.textAlign='left'; }
      ...
      if(toggles.watt) drawLine(wt, '#22c55e', yWatt);
      if(toggles.speed){ const ySpeed=v=> padT + (1 - (v - smin)/Math.max(1,(smax-smin))) * plotH; drawLine(sp, '#60a5fa', ySpeed); }
      if(toggles.rpe) drawLine(rp, '#facc15', yRPE);
    }
    drawGraph();

    // Laps table — include RPE per drag (bruk siste registrerte RPE i draget dersom rpeByRep mangler)
    ...
    function drawZones(canvas,z){ const ctx=canvas.getContext('2d'); const W=canvas.width, H=canvas.height; ctx.clearRect(0,0,W,H); const labels=[['Under LT1',z.under,'#14532d'],['LT1–LT2',z.between,'#1e3a8a'],['Over LT2',z.over,'#7f1d1d']]; const total=(z.under+z.between+z.over)||1; const barH=36; const gap=12; const pad=16; ctx.font=`${14*dpr}px system-ui`; ctx.fillStyle='#e5e7eb'; labels.forEach((row,i)=>{ const y = pad + i*(barH+gap); const frac = row[1]/total; const w = Math.max(1, Math.round((W-160*dpr)*frac)); ctx.fillStyle=row[2]; ctx.fillRect(140*dpr, y, w, barH); ctx.fillStyle='#e5e7eb'; ctx.fillText(`${row[0]}`, 8*dpr, y+barH*0.7); ctx.fillText(`${Math.round(row[1])} s`, (140*dpr)+w+8*dpr, y+barH*0.7); }); }
  ...
  }catch(e){ const card=document.getElementById('err-card'); const pre=document.getElementById('err-log'); card.classList.remove('hidden'); pre.textContent += (e&&e.stack)? e.stack : (e.message||String(e)); }
});
